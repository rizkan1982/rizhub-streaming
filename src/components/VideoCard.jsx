...
export default function VideoCard({ video }) {
  ...
  return (
    ...
    >
      ...
      <div className="relative overflow-hidden aspect-[2/3] md:aspect-[9/14] rounded-2xl">
        {imageUrl && !imageError ? (
          <>
            {/* Main Image */}
            <img
            ...
            {/* Preview Shimmer Effect (simulates video preview) */}
            {isHovering && (
        ...
        <button
          ...
          className="absolute top-3 right-3 w-10 h-10 bg-black/90 backdrop-blur-xl rounded-xl flex items-center justify-center hover:scale-110 transition-transform border border-white/10 z-10"
        ...
        >
          {isFav ? (
            ...
            <svg className="w-6 h-6 text-white/60 hover:text-red-400 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {/* Duration Badge */}
        {video.duration && (
          <div className="absolute bottom-3 right-3 bg-black/90 backdrop-blur-xl px-3 py-1.5 rounded-xl text-xs font-bold text-white border border-white/10">
            {video.duration}
          </div>
        )}
      ...
      <div className="p-4 space-y-3">
        <h2 className="text-sm font-bold line-clamp-2 text-white group-hover:text-gradient-rizhub transition-all duration-300">
