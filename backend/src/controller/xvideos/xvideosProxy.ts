...
export async function proxyXvideosStream(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    // Fetch video with enhanced headers untuk bypass ISP Indonesia (Telkomsel, XL, Indosat)
    const response = await fetch(videoUrl, {
      agent,
      timeout: 90000, // 90 detik untuk koneksi seluler lambat
      headers: {
        ...
        "Sec-CH-UA-Mobile": "?1", // Simulate mobile untuk better routing
    ...
    if (range && contentLength) {
      ...
      const rangeResponse = await fetch(videoUrl, {
        agent,
        timeout: 90000, // 90 detik untuk koneksi seluler
        headers: {
          "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
          "Referer": "https://www.xvideos.com/",
          "Origin": "https://www.xvideos.com",
